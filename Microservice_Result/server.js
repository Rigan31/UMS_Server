...
app.post('/result_file', async (req, res, next) => {
  ...
  console.log("offered course id is: ", course_id)
  console.log("session: ", session_id)

  offerCourseInfo = await client.execute('select id from offered_course_by_course_id where course_id = ?', [course_id])
  offered_course_id = offerCourseInfo.rows[0].id
...
app.get('/getresult', async(req, res, next)=>{
  ...
  for(let i = 0; i < result.length; i++){
    ...
    courseInfo = await client.execute('select course_title, course_label, credit from course where id = ?', [course_id])
